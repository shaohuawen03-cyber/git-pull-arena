...
- 2026-09-17：**本会话（`arena/01a0ad15`）Windows 原生肽 ML round 47 passed + accepted**：配方切 `peptide-ml`，无 bash 包装器 `run_peptide_ml.py` 按探针顺序自发现——首个达标即中 `E:\spider\Library\envs\NTxPred2\python.exe`（py3.10.7 numpy1.25/pandas1.5/sklearn1.5，与探针 best=NTxPred2 一致）；RandomForest（300/100 分层，种子 7）3.9 秒跑完，acc=0.97（P=1.00 R=0.70 F1=0.82，TN=90 FP=0 FN=3 TP=7），100 行 predictions.csv + metrics.json + run_report.txt（host=win32，at 14:49:08）推回覆盖 `results/peptide_ml/`（WSL 版在 `peptide_ml_linux/`），标准 172/0，`eaff2f7 accepted`。
- 2026-09-17：**本会话（`arena/01a0ad15`）Windows 原生肽探针 round 46 passed + accepted**：`local_check.ps1` §4 加 runner 分流（recipe≠office-deck → `local_loop.py local --os windows`，收据判 loop-ok + opened=）；rounds 42–45 连败四轮全是找 python 的问题——PATH 里只有 Store 桩（9009），`E:\spider` 才是真 conda base（22 环境，`Scripts\conda.exe` 在），PS5.1 调 native 会吞双引号导致 `-c` 在每台解释器上以同样方式全灭（探针改单引号即好，`print(''py-ok'')`）。round 46 venv runner（`ppt-master\.venv`）216 秒跑通：`envs=22 ml_ready=yes best=NTxPred2`（py3.10.7 numpy1.25/pandas1.5/sklearn1.5 + torch CPU；DIP、meta-analysis 也达标），`machine_probe_windows.json` 落仓并 pin 进成功标准（81 files），标准 170/0，`81c47fb accepted`。
- 2026-09-17：**本会话（`arena/01a0ad15`）round 41 Windows 原生轮 passed + accepted**：前置＝用户把 WSL cron 摘掉（同分支双值守会抢答）+ 浏览器把 `mqgg5630-cyber` 加为新仓库协作者（Windows 存的是旧号凭据，push 报 `permission denied`；加完两边各用各号，天然共存）。真机 57 秒跑完：门禁 5/5、7 份交付物 3a–3g 全 OK、3h 真 Word/PowerPoint 只读开档、§4 复用 `E:\0github\git-sync\ppt-master`（venv py 3.11.9）重建 umami deck（12/12 checker 通过，blocking=0）+ 4c 真 PowerPoint 开档，成功标准 165/165，`826c3d0 accepted`。插曲：沙箱 `.git` 又被静默 reset 到基线（远端 ref 丢失）→ `agent-recover.sh` 一键恢复，工作区零丢失；等待进程记录也丢了，改手动验 verdict 后 `--accept`。
- 2026-09-17：**本会话（`arena/01a0ad15`）肽 ML 预测 round 40 passed + accepted**：用户「用本地 conda 环境测试一个肽的机器学习预测」。动作：`code/run_peptide_ml.sh` 运行时自发现（探针报告 + 新鲜扫描，首个含 numpy/pandas/sklearn 者胜）→ 选中 `AMPidentifier`（py 3.11.15, sklearn 1.8.0）→ `code/peptide_ml.py` 跑 RandomForest（300/100 分层，AAC+电荷+疏水特征，SYNTHETIC DEMO 数据，断言 acc≥0.80）：真机 4 秒跑完，acc=0.97，`predictions.csv`（100 行）+ `metrics.json` + `run_report.txt` 推回，成功标准 165/165，`23656d6 accepted`。验完 `loop.json` 切回 `office-deck`。
- 2026-09-17：**本会话（`arena/01a0ad15`）round 39 探针 + 白名单补洞**：`code/machine_probe.py` 上报真机 13 个 conda 环境（`ml_ready=yes best=AMPidentifier`，另有 HMD-AMP/amp-esm/amppre/ilearnplus 等肽相关环境）。教训：`--accept` 把机器的 LAPTOP 探针报告埋掉了（`machine_probe.*` 不在白名单里，`2912aaa` 有真值、`a31602f` 被沙箱旧副本覆盖）→ 白名单加 `machine_probe.*` / `*_receipt.*` / `results/peptide_ml/*`（`agent-check.sh` + `pull_machine_evidence.sh` 两处，附 round-40 预置位），并从 `2912aaa` 恢复真值。
